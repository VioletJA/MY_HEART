...
//显示24*24数字
void OLED_ShowNUM24(uint8_t x,uint8_t y,uint8_t no)
{      			    
	uint8_t t;
	OLED_Set_Pos(x,y);	
    for(t=0;t<12;t++)
	{
		OLED_WR_Byte(NUM_24_24[3*no][t],OLED_DATA);
     }	
	OLED_Set_Pos(x,y+1);	
    for(t=0;t<12;t++)
	{	
		OLED_WR_Byte(NUM_24_24[3*no+1][t],OLED_DATA);
     }
	OLED_Set_Pos(x,y+2);	
    for(t=0;t<12;t++)
	{	
		OLED_WR_Byte(NUM_24_24[3*no+2][t],OLED_DATA);
     }	
}
